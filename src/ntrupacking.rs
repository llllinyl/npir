...
impl<'a> NtruRp<'a> {
    ...
    pub fn packing(&self, 
        ...
        let even = index;
        let odd = index + step;
        let mut cte = self.packing(log_num - 1, even, ct, y_constants);
        let cto = self.packing(log_num - 1, odd, ct, y_constants);
    ...
    pub fn packing_for(&self, 
        log_num: usize, 
        ct: &[PolyMatrixNTT<'a>], 
        y_constants: &Vec<PolyMatrixNTT<'a>>
    ) -> PolyMatrixNTT<'a> {
        let mut work_ct = Vec::new();
        for i in 0..self.ntru_params.poly_len{
            work_ct.push(ct[i].clone());
        }
        for i in 0..log_num{
            let step = 1 << (self.ntru_params.poly_len_log2 - i - 1);
            for j in 0..step{
                let mut ctl = work_ct[j].clone();
                let ctr = work_ct[j + step].clone();
                let y = &y_constants[i];

                let y_times_cto = scalar_multiply_alloc(&y, &ctr);
                let mut ct_sum_1 = ctl.clone();
                sub_into(&mut ct_sum_1, &y_times_cto);
                add_into(&mut ctl, &y_times_cto);
                let ct_sum_1_automorphed = homomorphic_automorph(
                    self.ntru_params,
                    (1 << (i + 1)) + 1,
                    &ct_sum_1,
                ...
                add_into(&mut ctl, &ct_sum_1_automorphed);
                work_ct[j] = ctl.clone();
            }
        }
        work_ct[0].clone()
...
mod tests {
    ...
    fn packing_correctness() {
        ...
        println!("packinging time: {} microseconds", micros1 / 5);
    }
