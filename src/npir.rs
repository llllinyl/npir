    ...
    pub fn uncompress(&self, column_cipher: PolyMatrixNTT<'a>) -> Vec<PolyMatrixNTT<'a>> {
        ...
        for level in 0..recursion_depth {
            ...
            for idx in 0..level_size {
                ...
                ciphertexts[target_idx].get_poly_mut(0, 0).copy_from_slice(negated_ct_ntt.get_poly(0, 0));
            }
        }
        ciphertexts
    }
...
mod tests {
    ...
    fn pir_correctness() {
        ...
        let npir = Npir::new(&ntru_params, 1, 3, 3, 8); // The second input is \phi; the third is t_pk; the fourth is t_g; the fifth is t_ce
    ...
    fn compress_query_correctness() {
        let ntru_params = Params::init(2048, 
            ...
            30,);
        println!("Generate the database with size 2^{} ...", ntru_params.db_size_log); 
        let npir = Npir::new(&ntru_params, 1, 3, 3, 8);
        println!("The database has {} rows and {} cols.", npir.drows, npir.ell); 
        let mut rng = ChaCha20Rng::from_entropy();
        let dimension = ntru_params.poly_len;
        ...
        for _t in 0..6 {
            ...
            let uncom = npir.uncompress(column);

            for i in 0..npir.ell {
                let qp = npir.ntrurp.decryptpoly(uncom[i].clone());
                // println!("The {}-the plaintext is {}", i, qp.get_poly(0, 0)[0]);
                if i != index_c / ntru_params.poly_len {
                    assert_eq!(qp.get_poly(0, 0)[0], 0);
                }
                else{
                    assert_eq!(qp.get_poly(0, 0)[0], 1);
                }
            }
            println!("Query compression and uncompression are ok!");

            let query = npir.queryrecovery(rotate, &uncom);
            for i in 0..npir.ell {
                let qp = npir.ntrurp.decryptpoly(query.submatrix(i, 0, 1, 1));
                if i != index_c / ntru_params.poly_len {
                    for j in 0..ntru_params.poly_len{
                        assert_eq!(qp.get_poly(0, 0)[j], 0);
                    }
                    // println!("The {} query is 0!", i);
                }
                else{
                    for j in 0..ntru_params.poly_len{
                        if j != rotation_offset {
                            assert_eq!(qp.get_poly(0, 0)[j], 0);
                        }
                        else {
                            if col_index == 0{
                                assert_eq!(qp.get_poly(0, 0)[rotation_offset], 1);
                            }
                            else{
                                assert_eq!(qp.get_poly(0, 0)[rotation_offset], ntru_params.pt_modulus - 1);
                            }
                        }
                    }
                    // println!("The {} query is the correct rotation query!", i);
                }
            }
        }
        println!("Query (un)compression and recovery are ok!");
    }
}